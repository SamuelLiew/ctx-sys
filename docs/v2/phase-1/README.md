...
| F1.5 | [Retrieval eval suite](F1.5-retrieval-eval-suite.md) | High | No |
| F1.6 | [Ollama UX](F1.6-ollama-ux.md) | High | No |

F1.0 drops 7 of the 12 MCP tools (6 conversation + `hooks`). ADRs move to plain markdown — ctx-sys's document indexer already handles them. No reframed `decision` node; no special pipeline. F1.5 lands the eval harness that future retrieval changes (cutting the heuristic reranker, etc.) will be measured against.
...
- Land F1.5 (eval suite) first — it's the gate that lets every subsequent change be measured.
- Land F1.1, F1.2, F1.6 next — low-risk, unblock new-user onboarding.
