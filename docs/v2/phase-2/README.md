...
- **Error UX** — every user-facing `CtxError` carries a `fix:` hint; bare `throw new Error(...)` paths in the CLI are lifted into the structured error system.
...
| F2.1 | [Error message + hint audit](F2.1-error-hints.md) | Medium | No (content-only) |
...
- **F2.1 next.** Cheap audit that newly-introduced F2.0 error paths (`STALE_INDEX_*`, hook-install conflicts) want to participate in. Lifting bare `throw new Error(...)` calls also makes the CLI more consistent before F2.2 adds preflight surfaces. → 2.1.x (can ride 2.1.0).
...
- **F2.1** depends on F1.0 having removed obsolete error codes (`SESSION_NOT_FOUND` and friends) so the audit isn't chasing dead paths, and on F1.4's `--json` error shape being the canonical serialisation for `CtxError.toMcpResponse()`.
